---
layout: post
title: "시계열 분석 시리즈 (2): AR / MA / ARIMA 모형 "
date: 2021-08-23
...
### Case 1. 백색 잡음 모형 (White noise model)

> $y_t = \epsilon_t$ 과거의 값과 현재의 값의 상관관계가 없는, 단순한 Shock 형태

$\phi_1 = 0$일 때 $y_t$는 **백색잡음**입니다. 백색잡음 모형은 위에서 설명했듯이, 과거의 값으로 현재의 값을 예측할 수 없는 랜덤한 상태를 의미합니다.

### Case 2. 확률보행 모형 (Random walk model)
...
### Case 3. 표류가 있는 확률보행 모형 (Random walk model with a drift)

> $y_t = c+ y_{t-1} + \epsilon_t$, 시간이 지남에 따라 평균적으로 값이 증가하거나 감소하는 형태


$\phi_1 = 1$, $c\ne 0$이면 $y_t = c+ y_{t-1} + \epsilon_t$로, 여기서 상수 $c$를 표류 (drift)라 합니다. 그래서 이 모형은 **표류가 있는 확률 보행 (Random walk with a drift)** 모형입니다.
...
### Case 4. 정상성을 띠는 AR(1) 모형

> $y_t = c + \phi_1 y_{t-1} + \epsilon_t$, 정상성 (Stationarity)을 띠는 AR(1) 모형

$\phi_1 < 0$이면 정상성 (Stationarity)을 띠는 AR (1)모형입니다.

---
### 확률 보행 모형은 정상성을 띠지 않는다.

여기서 Case 1 ~ Case 4에서 정상성을 띠는 모형은 Case 1과 Case 4입니다. AR(1)의 모형에서 정상성을 띨 조건은 $-1 < \phi_1 < 1$이고, 확률 보행 모형의 $\phi_1$값은 1이기 때문에 그렇습니다.

그런데 왜 정상성을 띠지 않을까요? 이 확률보행 모형은 **분산이 시간이 지남에 따라 증가하여 $y_t$의 분포가 시간에 따라 변하기 때문입니다.** 참고로 정상성에 대한 이해가 부족하시다면 [시계열 분석 시리즈 (1): 정상성 (Stationarity) 뽀개기](https://assaeunji.github.io/ml/2021-08-08-stationarity/){:target='_blank'}를 참조해주시길 바랍니다.

분산이 시간이 지남에 따라 왜 증가하는지 수식을 통해 알아보겠습니다. 다음과 같은 확률보행 모형을 가정합니다.
...
참고로, $\epsilon_t$는 백색잡음으로, iid한 성질을 가지기 때문에 $Var(\sum_{t=1}^T \epsilon_t) = \sum_{t=1}^T Var (\epsilon_t)$로 쓸 수 있습니다.

그리하여 $y_t$의 분산은 $t$에 의존하게 되어 $t$가 증가함에 따라 (시간이 지남에 따라) 분산이 커지게 됩니다. 따라서 정상성의 정의에 따라서 분산이 유한하지 않아 정상성을 띠지 않게 됩니다.

---
### AR 모형은 정상성이 있는 시계열에 써야한다!
...
과 같은 결과를 얻습니다. 또한, 분산은 무조건 0보다 크거나 같아야 하므로 $\phi_1^2$이 반드시 1보다 작아야 합니다. 이는 정상과정에서 $\phi_1$의 범위가 $-1 < \phi_1 < 1$이 되어야한다는 의미입니다.


---
## 이동 평균 모델 (MA; Moving Average Model)

> 과거의 충격 (Shock)이 현재의 값에 영향을 줄 때
...
신기하지 않나요? AR(1) 모형에선 정상성을 띠기 위해 $-1<\phi_1<1$ 조건이 필요했는데,
정의상 MA 모델은 어떠한 모수에 제약이 없어도 약한 정상성을 띱니다.
...
> 주식처럼 충격(Shock)에도 민감하고, 과거의 값에도 영향을 받는 총체적 난국일 떄

![](../../images/arima-arisu.jpg)
아리수 아닙니다. (이쯤되면 지겨우니 대충 아무 짤이나...)
{:.figure}

드디어 대망의 ARIMA 모델이 왔습니다! ARIMA 모델은 AR 모형 부분도 있고, MA 부분도 있는데, 차분까지 한 PPAP의 진화 버전 PPPAP (ㅃ...빱-!)의 형태라 할 수 있습니다. ARIMA (p,d,q) 모델은 다음과 같이 정의됩니다.

$$
y'_t = c + \phi_1 y'_{t-1} + \cdots + \phi_p y'_{t-p} + \theta_1 \epsilon_1 + \cdots + \theta_q \epsilon_{t-q} + \epsion_t
$$
