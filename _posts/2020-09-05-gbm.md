---
layout: post
title: "GBM (Gradeint Boosting Machines)에 대한 자세한 설명 (1): Regression"
...
여기서 $\eta$는 학습률 (learning rate), 스텝의 크기 (step size)라 불리우는데, 한 번 업데이트할 때 얼마나 큰 크기로 업데이트할지 조정하는 값으로 사용자에 의해 지정되는 값입니다.
...
3. 이 잔차에 깊이가 2인 나무모형을 적합해 잔차 예측값인 $\gamma$를 구합니다.
...
2~4 과정을 내가 정한 반복수 $T$만큼 반복하면 다음과 같이 GBM을 구현할 수 있습니다.
...
* Deep Play님의 Gradient Boosting Algorithm의 직관적 이해 
  [`[link]`](https://3months.tistory.com/368)
