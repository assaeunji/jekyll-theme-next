...
개수, 합계, 최소, 최대, 평균을 각 컬럼에 할당하여 저장하려면 `agg` 함수로 묶어서 사용합니다.
```python
...
```python
# INNER JOIN
empDF.join(deptDF, empDF.emp_dept_id == deptDF.dept_id).show(truncate=False)
# FULL JOIN
empDF.join(deptDF, empDF.emp_dept_id == deptDF.dept_id, how = "full").show(truncate=False)
# LEFT JOIN
empDF.join(deptDF, empDF.emp_dept_id == deptDF.dept_id, how = "left").show(truncate=False)
```

만약 조인키 이름이 같다면 어떻게 할까요? 
공통된 key `dept_id` 를 `on` 조건에 적어주면 됩니다.
...
중복된 행을 허용하여 union을 해줍니다. (즉, `UNION ALL`과 같습니다)
...
`union`은 기본적으로 컬럼의 개수가 같아야 하며, 컬럼 순서도 동일하게 두어야 제대로된 결과를 얻을 수 있습니다.
...
df.withColumn('row_num', row_number().over(Window.partitionBy('Product').orderBy(desc('Amount')))).show()
```
