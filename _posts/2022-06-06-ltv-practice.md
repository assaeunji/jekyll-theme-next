...
구체적인 R, F, M, T는 [이전 포스팅](https://assaeunji.github.io/statistics/2022-04-15-ltv/){:target='_blank'}를 참조해주셔도 되고, 이번 포스팅에서도 예시로 구해볼 예정이니 걱정 마세요~!


---
## Online Retail 데이터 살펴보기

먼저 저희가 LTV를 계산할 때 필요한 모듈은 다음과 같습니다. 여기서 `lifetimes` 모듈이 바로 LTV를 구할 때 필요한 모듈이니 기억해주세요!
...
* InvoiceDate (주문 일자): Datetime -> date형으로 변환
* CustomerID: NULL인 것 제외
* Quantity (주문 수량): 1 이상인 것만 가져오기
* Sales (구매 금액) 변수 생성
* 고객 번호, 주문 일자, 구매 금액만 남기고 지우기
...
단, 그림에서 보이듯이 한 고객이 하루에 여러 번 구매했을 경우 굳이 일자별로 구매 금액을 합해 한 행으로 남길 필요는 없고, 그대로 두어도 괜찮습니다.
...
이 네 변수의 의미를 잘 안다면 수동으로 만드는 것도 가능하지만 lifetimes의 `summary_data_from_transaction_data` 함수를 이용해 쉽게 만들 수 있습니다.
