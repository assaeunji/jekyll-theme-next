...
* 이 글은 엄격한 실험이 아닌 관측 환경 (observational studies)에서 **인과관계 추론**을 위해 사용하는 Bayesian Network와 Potential Outcomes 방법론에 대한 글입니다.
...
$$P(X\vertY=y) = \frac{P(X, Y=y)}{P(Y=y)} = \frac{P(Y\vertX) P(X)}{\sum_{x\in X} P(Y\vertX=x)P(X=x)}$$

조건부 확률에서 분자는 X와 Y의 결합 확률 분포 (joint probability distribution), 분모는 주변부 확률 분포 (marginal probability distribution)라 합니다. 

이를 확장해서 Bayesian Network에서는 여러 인과 관계를 **조건부 확률**로 표현합니다. Bayesian Network의 두 번째 요소인 $\Theta$는 변수 $X_i$의 모든 가능한 값 $x_i$에 대한 $\theta_{x_{i} \vert \Pi_{x_{i}}}=P_{B}(x_{i} \vert \Pi_{x_{i}})$의 확률을 포함합니다. 여기서 $\Pi_{x_i}$는 DAG에서 표현된 $X_i$의 직계 부모의 집합을 의미합니다. 이를 통해 Bayesian Network $B$는 다음의 결합 확률 분포를 표현할 수 있습니다.

$$
P_{B}\left(X_{1}, \ldots, X_{n}\right)=\prod_{i=1}^{n} P_{B}\left(X_{i} \vert \Pi_{X_{i}}\right)=\prod_{i=1}^{n} \theta_{X_{i} \vert \Pi_{X_{i}}}
$$

많이 어렵죠... 그러나 실제 값을 표현하면 좀 쉽습니다! 예를 들어 $$X_1 \rightarrow X_2 \rightarrow X_3$$의 관계라면, $$P_B(X_1,X_2,X_3) = P_B(X_1) P_B(X_2\vertX_1) P_B(X_3\vertX_2)$$로 표현됩니다. $$X_1$$의 부모는 없고, $$X_2$$의 직계 부모는 $$X_1$$뿐이고, $$X_3$$의 직계 부모는 $$X_2$$뿐입니다. 따라서 결합 확률 분포에서 첫번째 확률은 $$\theta_{X_{1}} \vert\Pi_{X_{1}} = P_B(X_1)$$이 되고, 두번째 확률은 $$\theta_{X_{2}} \vert\Pi_{X_{2}} = P_B(X_2\vertX_1)$$이 되는 식입니다.
...
$$
\begin{aligned}
P(C\vertA,B,D,E) &= \frac{P(C,A,B,D,E)}{P(A,B,D,E)}\\
\text{where}\ P(C,A,B,D,E) &= P(A) \cdot P(B\vertA) \cdot P(C\vertA,B) \cdot P(D\vertC,E) \cdot P(E),\\
...
&= P(A\in[20;100)) \cdot P(B=0\vertA\in[20;100)) \cdot P(C=0\vertA\in[20;100),B=0) \cdot P(D \in[0 ; 130)\vertC=0,E=1) \cdot P(E=1)\\
&=0.5 \cdot 0.7 \cdot 0.85 \cdot 0.2 \cdot 0.45 = 0.0268\\
&P(C=1, A \in[20 ; 100), B=0, D \in[0 ; 130), E=1) \\
&= P(A\in[20;100)) \cdot P(B=0\vertA\in[20;100)) \cdot P(C=1\vertA\in[20;100),B=0) \cdot P(D \in[0 ; 130)\vertC=1,E=1) \cdot P(E=1)\\
...
1. Discrete Bayesian Network: 노드 $$X_i\vert\Pi_{X_{i}}$$가 이산형인 경우를 의미합니다. 지금까지의 예시가 모두 discrete한 예시입니다. 즉 노드마다 확률이 카테고리 별로 테이블 형태로 제시된 경우를 의미합니다.$$i$$번째 노드 $$X_i$$가 $$k$$번째 범주를 가지고 이 노드의 부모들의 값이 $$j$$라 할 때 조건부 확률은 다음과 같이 정의됩니다.

   $$
   \pi_{i k \vert j}=\mathrm{P}\left(X_{i}=k \vert \Pi_{X_{i}}=j\right)
   $$

2. Gaussian Bayesian Network: 노드 $$X_i\vert\Pi_{X_{i}}$$이 연속형일 때 쓰는 Bayesian Network입니다. 이때 $$i$$번째 노드 $$X_i$$는 다음과 같이 선형회귀 식으로 표현됩니다.
   $$
...
3. Conditional Gaussian Bayesian Networks (CDBN): 노드 $$X_i\vert\Pi_{X_{i}}$$가 범주형과 연속형이 섞여있는 경우 쓰는 Bayesian Network입니다. 이 때 $$X_i$$가 이산형이냐, 연속형이냐에 따라서 식이 다릅니다.
...
$$
P(B, E\vertMB(B)) = P(B, D\vertMB(B))=P(B, F\vertMB(B))=P(B\vertMB(B)),\text{where }MB(B) = \{A,C\}
$$
...
이 경우에 결합 확률에 $$P(Sprinkler\vertSeason)$$을 포함시켜 곱해주면 되겠죠. 이처럼 Bayesian Network는 개입이 있어도 확률의 조정을 해주면 됩니다.
...
**Potential Outcomes**는 일어나지 않은 결과를 고려해서 처리 효과를 계산하는 방식입니다. **do-calculus**라고도 표현하는 이 방식은 "만약 처리를 이렇게 줬을 때 어떤 결과가 나올까?"라는 궁금증을 해결하고자 $$p(y\vertdo(x))$$의 개념을 도입합니다. (참조: Ferenc Huszár님의 블로그 [`[link]`](https://www.inference.vc/untitled/)) 
...
"특정한 가정 하에" CATE와 CATT의 인자 중 $$E[Y_i(1)\vertX_i]$$와 $$E[Y_i(0)\vertX_i]$$를 다음과 같이 쓸 수 있습니다.

$$
\begin{aligned}
E[Y_i(1)\vertX_i] &= E[Y_i\vertX_i,Z_i=1]\\
E[Y_i(0)\vertX_i] &= E[Y_i\vertX_i,Z_i=0]
\end{aligned}
$$
...
1. (Positivity / Overlap) $$ 0<P(Z=1\vertX) <1$$
