...
* 인과 추론에 대한 resource를 더 확인하시려면 Shubhanshu Mishra님의 Awesome Causality [`[link]`](https://shubhanshu.com/awesome-causality/){:target="_blank"}를 추천합니다.
...
**Potential Outcomes**는 일어나지 않은 결과를 고려해서 처리 효과를 계산하는 방식입니다. **do-calculus**라고도 표현하는 이 방식은 "만약 처리를 이렇게 줬을 때 어떤 결과가 나올까?"라는 궁금증을 해결하고자 $$p(y\vert do(x))$$의 개념을 도입합니다 (참조: Ferenc Huszár님의 블로그 [`[link]`](https://www.inference.vc/untitled/){:target="_blank"}).
