...
* 이 글은 엄격한 실험이 아닌 관측 환경 (observational studies)에서 **인과관계 추론**을 위해 사용하는 Bayesian Network와 Pote.
...
많이 어렵죠... 그러나 실제 값을 표현하면 좀 쉽습니다! 예를 들어 $$X_1 \rightarrow X_2 \rightarrow X_3$$의 관계라면, $$P_B(X_1,X_2,X_3) = P_B(X_1) P_B(X_2\vert X_1) P_B(X_3\vert X_2)$$로 표현됩니다. $$X_1$$의 부모는 없고, $$X_2$$의 직계 부모는 $$X_1$$뿐이고, $$X_3$$의 직계 부모는 $$X_2$$뿐입니다. 따라서 결합 확률 분포에서 첫번째 확률은 $$\theta_{X_{1}} \vert \Pi_{X_{1}} = P_B(X_1)$$이 되고, 두번째 확률은 $$\theta_{X_{2}} \vert \Pi_{X_{2}} = P_B(X_2\vert X_1)$$이 되는 식입니다.
...
$$
P(B, E\vert MB(B)) = P(B, D\vert MB(B))=P(B, F\vert MB(B))=P(B\vert MB(B)),\text{where }MB(B) = \{A,C\}
$$
