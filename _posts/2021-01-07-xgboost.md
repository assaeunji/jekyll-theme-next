...
---
* 오늘은 [GBM에 대한 자세한 설명](https://assaeunji.github.io/ml/2020-09-05-gbm/)에 이어 GBM 기반의 XGBoost와 LightGBM 알고리즘에 대해 알아보고, 어떤 알고리즘이 더 좋은지 비교하고자 합니다.
...
따라서 이 포스팅에서는 XGBoost와 LightGBM을 파이썬의 사이킷런 (Scikit-liearn) API로 구현하고 이를 이탈 예측 데이터를 이용해 이탈을 예측해보고자 합니다.
...
이제 XGBoost를 아나콘다 환경에서 설치해보겠습니다. mac에서 터미널 창을 열어 다음과 같이 명령어를 입력합니다.
...
이에 대한 자세한 설명은 [lightGBM / XGBoost 파라미터 설명](http://machinelearningkorea.com/2019/09/29/lightgbm-파라미터/)를 참조해주시길 바랍니다.
...
[Which algorithm takes the crown: Light GBM vs XGBOOST?](https://www.analyticsvidhya.com/blog/2017/06/which-algorithm-takes-the-crown-light-gbm-vs-xgboost/)에 따르면, 리프 중심 분할 방법은 복잡도가 증가하고 과적합 문제가 발생할 수 있다 합니다. 그러나, 이는 트리의 최대 깊이인 `max_depth`의 하이퍼 파라미터를 조정한다면 해결할 수 있는 문제라 합니다.
...
마찬가지로, 중요도 순으로 정렬하였고 자세한 내용은 [lightGBM / XGBoost 파라미터 설명](http://machinelearningkorea.com/2019/09/29/lightgbm-파라미터/)를 참조해주시길 바랍니다.

과적합 방지를 위한 하이퍼 파라미터 튜닝 방안은 다음과 같습니다.

* learning_rate를 작게 하면서 n_estimators 크게 해봅니다.
...
이번에는 캐글의 [산탄데르 고객 만족 데이터 세트](https://www.kaggle.com/c/santander-customer-satisfaction)에 대해서 고객 만족 여부를 XGBoost와 
LightGBM을 통해 예측해보겠습니다.

산탄데르 고객 만족 예측 분석은 370개의 피처로 주어진 데이터 세트 기반에서 고객 만족 여부를 예측하는 것입니다. 클래스 레이블 명은 TARGET이고, 1이면 불만을 가진 고객, 0이면 만족한 고객을 의미합니다.

대부분 만족으로 응답한 고객이 많기 때문에 정확도 수치보단 ROC-AUC가 더 적합하기 때문에 평가 메트릭은 ROC-AUC로 하였습니다.
...
그 결과 훈련 데이터의 $X$ 크기는 (60816, 369)가 됩니다. 
